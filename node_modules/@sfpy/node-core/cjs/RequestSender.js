    ...
    _jsonResponseHandler(callback) {
        return (res) => {
            ...
            res
                ...
                let err;
                if (statusCode === 401) {
                    err = new Error_js_1.SafepayAuthenticationError(message);
                }
                else if (statusCode === 400) {
                    err = new Error_js_1.SafepayInvalidRequestError(message);
                }
                else if (statusCode === 404) {
                    err = new Error_js_1.SafepayAPIError(message);
                }
                else if (statusCode === 409) {
                    err = new Error_js_1.SafepayConflictError(message);
                }
                else {
                    err = new Error_js_1.SafepayUnknownError(message);
                }
                throw err;
            }, (e) => {
                throw e;
